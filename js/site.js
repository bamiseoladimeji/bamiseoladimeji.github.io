...
function render(d){
 ...
 const quotes=d.testimonials.map(t=>`<article class="quote reveal">${t.photo&&t.photoEnabled!==false?`<img class="quote-photo" src="${esc(t.photo)}" alt="${esc(t.name)}">`:''}<blockquote>“${esc(t.quote)}”</blockquote><small>${esc(t.name)}${t.role?' · '+esc(t.role):''}</small></article>`).join('');
 ...
 <header class="nav"><div class="container nav-inner"><a class="logo" href="#home">${logo}</a><nav class="links">${nav}</nav><a class="hire" href="${esc(header.hireUrl||'#contact')}">${esc(header.hireLabel||'Hire Me')} →</a></div></header>
 <main>
 <section class="hero container ${heroPortrait?'':'no-portrait'}" id="home"><div><div class="eyebrow">${esc(d.hero.eyebrow)}</div><h1>${esc(d.hero.title)}</h1><p>${esc(d.hero.description)}</p><div class="actions"><a class="btn primary" href="${esc(d.hero.primaryCtaUrl||'#work')}">${esc(d.hero.primaryCta)} →</a><a class="btn" href="${esc(d.hero.secondaryCtaUrl||'#contact')}">${esc(d.hero.secondaryCta)}</a></div></div>${heroPortrait}</section>
 <section class="section container" id="work"><div class="section-head"><div><div class="eyebrow">${esc(workSection.eyebrow||'Featured Work')}</div><h2>${esc(workSection.title||'Selected Projects')}</h2></div>${viewAllUrl&&viewAllUrl!=='#work'?`<a class="small-link" href="${esc(viewAllUrl)}" target="_blank" rel="noreferrer">${esc(viewAllLabel)} →</a>`:`<a class="small-link" href="#work">${esc(viewAllLabel)} →</a>`}</div><div class="grid">${projects}</div></section>
 ...
 <section class="section container" id="services"><div class="section-head"><div><div class="eyebrow">${esc(servicesSection.eyebrow||'What I Do')}</div><h2>${esc(servicesSection.title||'Services')}</h2></div></div><div class="services">${services}</div></section>
 <section class="section container" id="testimonials"><div class="section-head"><div><div class="eyebrow">${esc(testimonialsSection.eyebrow||'Kind Words')}</div><h2>${esc(testimonialsSection.title||'Testimonials')}</h2></div></div><div class="quote-grid">${quotes}</div></section>
 <section class="section container" id="contact"><div class="contact"><div><div class="eyebrow">${esc(contactSection.eyebrow||"Let's Work Together")}</div><h2>${esc(contactSection.title||'Have a project in mind?')}</h2><p class="contact-copy">${esc(d.site.availability)}<br>${esc(contactSection.description||"Let's talk about what we can create.")}</p></div><div><div class="contact-list"><div class="contact-row"><span>Email</span><a href="mailto:${esc(d.site.email)}">${esc(d.site.email)} ↗</a></div><div class="contact-row"><span>Location</span><span>${esc(d.site.location)}</span></div>${socials?`<div class="contact-row"><span>Social</span><span style="display:flex;gap:18px">${socials}</span></div>`:''}</div></div></div></section>
