...
function normalize(){state.site ||= {};state.hero ||= {};state.projects ||= [];state.testimonials ||= [];state.about ||= {stats:[]};state.services ||= [];state.socials ||= [];state.navigation ||= [];state.settings ||= {};state.logo ||= {enabled:false,image:''};state.footer ||= {enabled:true,text:'Designed & built with intention.',copyright:'© {year} {name}',showNavigation:true,showSocials:true};state.projectSection ||= {viewAllLabel:'View All Projects',viewAllUrl:'#work'};state.projectSection.viewAllLabel ||= 'View All Projects';state.projectSection.viewAllUrl ??= '#work';state.hero.portraitEnabled = state.hero.portraitEnabled!==false;state.hero.heroImage ||= '';state.logo.enabled=state.logo.enabled===true;state.projects.forEach(p=>{p.gallery ||= [];p.viewMoreLabel ||= 'View More';p.viewMoreUrl ??= '';delete p.link;});}
