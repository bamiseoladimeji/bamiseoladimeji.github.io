...
function render(){
 $('#pageTitle').textContent=({site:'Homepage',projects:'Projects',testimonials:'Testimonials',footer:'Footer',about:'About',services:'Services',theme:'Theme',advanced:'Advanced JSON',overview:'Overview'})[tab]||tab;
 const p=$('#panel');
 ...
 <div class="card"><h2>Homepage — identity</h2>
 ${input('Name','site.name',val('site.name'))}${input('Short name / fallback logo','site.shortName',val('site.shortName'))}${input('Professional title','site.role',val('site.role'))}${input('Email','site.email',val('site.email'))}${input('Location','site.location',val('site.location'))}${input('Availability','site.availability',val('site.availability'))}
 </div>
 ...
 if(tab==='projects')p.innerHTML=`<div class="card"><h2>Projects section</h2>${input('View All Projects label','projectSection.viewAllLabel',val('projectSection.viewAllLabel'))}${input('View All Projects URL','projectSection.viewAllUrl',val('projectSection.viewAllUrl'))}<p class="muted">Use a Pinterest or Behance URL to send visitors to your wider portfolio. Leave the URL as <code>#work</code> if you want the button to stay on the page.</p></div><div class="card"><div class="addbar"><div><h2>Projects</h2><p class="muted">Add, edit or delete projects. Each project can have as many additional gallery images as you need, each with its own editable title.</p></div><button class="edit" type="button" onclick="addProject()">+ Add Project</button></div>${state.projects.map((x,i)=>projectEditor(x,i)).join('')}</div>`;
...
window.addProject=()=>{collect();state.projects.push({id:uid('project'),title:'New Project',category:'Category',year:String(new Date().getFullYear()),description:'Project description.',image:'assets/project-01.svg',gallery:[],featured:false,viewMoreLabel:'View More',viewMoreUrl:''});render();notice('Project added. Fill it in, then Save to GitHub.')}
